...
function Single() {
  ...
  return (
    <div>
      <HeroSection page={` ${id}`} image={image} />
      <Container>
        {data ? (
          <div
            className="relative mt-[-100px] sm:mt-[-250px] mb-[55px] text-white grid sm:grid-cols-[repeat(auto-fill,minmax(280px,1fr))] grid-cols-[repeat(auto-fill,minmax(200px,1fr))] gap-x-[20px] gap-y-[30px] font-poppins">
            {data.map((el) => {
              return (
                <div
                  data-aos="fade-up"
                  ...
                  key={el.id}
                  className="relative mx-auto bg-[#262626] transition-all w-full max-w-[300px]"
                >
                  <div className="absolute top-[20px] left-[50%] translate-x-[-50%] bg-[#262626] flex items-center w-[75%] sm:text-sm text-xs">
                    <p className="bg-primary p-[5px] w-[50%] text-center text-black font-medium">
                      Unit Price
                    </p>
                    <p className="p-[5px] w-[50%] text-center">{el.price}</p>
                  </div>
                  ...
                  <div className="px-[20px] py-[15px]">
                    <h2 className="sm:text-xl font-bold">{el.title}</h2>
